...
router.get("/filters", async (req, res) => {
  try {
    ...
    const locations = await Customer.distinct("location");
    const callingBy = await Customer.distinct("callingBy");
    const attendingBy = await Customer.distinct("attendingBy");
    const status = await Customer.distinct("status");

    res.json({ villages, locations, callingBy, attendingBy, status });
  } catch (err) {
